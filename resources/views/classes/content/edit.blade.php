    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('classes.content.update', [$class, $content]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            <select name="week_number" id="week_number" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                ...
                                @foreach($weeks as $week)
                                    <option value="{{ $week }}" {{ old('week_number', $content->week_number) == $week ? 'selected' : '' }}>
                                        Week {{ $week }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="content" id="content" rows="12"
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                placeholder="Enter rich content here. You can use HTML formatting or plain text.">{{ old('content', $content->content) }}</textarea>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="additional_notes" id="additional_notes" rows="3"
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                placeholder="Any additional notes or instructions">{{ old('additional_notes', $content->additional_notes) }}</textarea>
                                ...
                                @if($content->attachments && count($content->attachments) > 0)
                                    @foreach($content->attachments as $attachment)
                                        <div class="flex space-x-2">
                                            <input type="url" name="attachments[]" value="{{ $attachment }}"
                        ...
                        <div class="mb-6">
                            <label class="flex items-center">
                                ...
                                <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">
                                    Publish (visible to enrolled members)
                                </span>
                            </label>
                        </div>
