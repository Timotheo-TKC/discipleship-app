        ...
        <div>
            ...
            @if($user->isAdmin())
                <select id="role" name="role" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                    @foreach(\App\Models\User::getRolesWithLabels() as $roleKey => $roleName)
                        <option value="{{ $roleKey }}" {{ old('role', $user->role) === $roleKey ? 'selected' : '' }}>{{ $roleName }}</option>
                    @endforeach
                </select>
            @else
                <x-text-input id="role" type="text" class="mt-1 block w-full bg-gray-100 dark:bg-gray-800" :value="\App\Models\User::getRolesWithLabels()[$user->role]" readonly />
                <input type="hidden" name="role" value="{{ $user->role }}">
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Contact an administrator to change your role.</p>
            @endif
